...
def parse_data(i, researcher):
    ...
    if i > 22:
        anchor_elem = anchor_elem[0].get_text().replace("\xa0", " ")
    else:
        anchor_elem = anchor_elem[len(anchor_elem) - 1].get_text().replace("\xa0", " ")
...
def main():
    ...
    results_path = os.path.join(
        get_project_root(), "data", "researchers", "uklo", "pedagoski"
    )
    data = []
    for url in urls:
        ...
        data.extend(
            [parse_data(i, researcher) for i, researcher in enumerate(staff) if i != 0]
        )
