...
def parse_data(researcher, last):
    researcher_name = researcher.get_text().replace("\xa0", " ")
    if last:
...
def main():
    ...
    results_path = os.path.join(
        get_project_root(), "data", "researchers", "ukim", "pedagoski"
    )
    ...
    data = [
        parse_data(researcher, i == len(staff) - 1)
        for i, researcher in enumerate(staff)
    ]
